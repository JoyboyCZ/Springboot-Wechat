	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="运动类型" prop="yundongleixing" >
				...
				<el-form-item class="select" v-if="type!='info'"  label="训练频率" prop="xunlianpinlv" >
					<el-select :disabled="ro.xunlianpinlv" v-model="ruleForm.xunlianpinlv" placeholder="请选择训练频率" >
						<el-option
							v-for="(item,index) in xunlianpinlvOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="训练频率" prop="xunlianpinlv" >
					<el-input v-model="ruleForm.xunlianpinlv"
						placeholder="训练频率" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="训练天数" prop="xunliantianshu" >
					<el-input v-model="ruleForm.xunliantianshu" placeholder="训练天数" clearable  :readonly="ro.xunliantianshu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="训练天数" prop="xunliantianshu" >
					<el-input v-model="ruleForm.xunliantianshu" placeholder="训练天数" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="预期目标" prop="yuqimubiao" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="预期目标"
					v-model="ruleForm.yuqimubiao" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yuqimubiao" label="预期目标" prop="yuqimubiao" >
				<span class="text">{{ruleForm.yuqimubiao}}</span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='xunliantianshu'){
							this.ruleForm.xunliantianshu = obj[o];
							this.ro.xunliantianshu = true;
							continue;
						}
						if(o=='jihuaxiangqing'){
				...
				this.xunlianpinlvOptions = "一周一次,一周两次,一周三次,一周四次,一周五次,一周六次,一周七次".split(',')
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jihuaxiangqing = this.ruleForm.jihuaxiangqing.replace(reg,'../../../springbootltn8n5j0/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jianshendingzhi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jianshendingzhiCrossAddOrUpdateFlag = false;
											this.parent.search();
