		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","order":"2"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="建议类型" prop="jianyileixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">建议类型</label>
						<el-select clearable v-model="searchForm.jianyileixing" placeholder="请选择建议类型" >
							<el-option v-for="(item,index) in jianyileixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">封面</label>
						<el-input v-model="searchForm.fengmian" placeholder="封面" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yinshijianyi','私聊')" type="success" @click="chatClick(scope.row)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								私聊
							</el-button>
							<el-button class="view" v-if="isAuth('yinshijianyi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init () {
				this.$http({
					url: `option/jianyileixing/jianyileixing`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.jianyileixingOptions = data.data;
					} else {
			...
			getDataList() {
				...
				if(this.searchForm.jianyileixing!='' && this.searchForm.jianyileixing!=undefined){
					params['jianyileixing'] = this.searchForm.jianyileixing
				}
 				if(this.searchForm.fengmian!='' && this.searchForm.fengmian!=undefined){
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussyinshijianyi',query:{refid:id}});
			},
			// 删除
