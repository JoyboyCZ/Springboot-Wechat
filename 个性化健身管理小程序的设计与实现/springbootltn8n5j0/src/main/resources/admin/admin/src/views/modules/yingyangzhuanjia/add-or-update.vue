	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					yingyangshizhanghao: [
						{ required: true, message: '营养师账号不能为空', trigger: 'blur' },
					],
					mima: [
						{ required: true, message: '密码不能为空', trigger: 'blur' },
					],
					yingyangshixingming: [
						{ required: true, message: '营养师姓名不能为空', trigger: 'blur' },
					],
					xingbie: [
					],
					lianxidianhua: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yingyangzhuanjia/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yingyangzhuanjiaCrossAddOrUpdateFlag = false;
											this.parent.search();
