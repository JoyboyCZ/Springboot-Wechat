	...
	<view class="content">
		<view class="container" :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
			...
			<view :style='{"width":"100%","padding":"24rpx","background":"none","height":"auto"}' class="detail-content">
				...
				<view class="detail-list-item" :style='{"margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}'>联系电话：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#333"}' style="text-decoration: underline" @tap="callClick(detail.lianxidianhua)" >{{detail.lianxidianhua}}</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","flexWrap":"wrap","background":"none","display":"flex","gap":"16rpx","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","borderRadius":"8rpx","background":"#665682","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('yingyangzhuanjia','私聊')" @tap="chatClick">私聊</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","borderRadius":"8rpx","background":"#665682","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('yingyangzhuanjia','私聊')" @tap="chatClick">私聊</button>
	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('yingyangzhuanjia', this.id);
				let reg=new RegExp('http://localhost:8080/springbootltn8n5j0/upload','g')//g代表全部
				this.detail = res.data;
				this.title = this.detail.yingyangshixingming
			}
		},
		destroyed: function() {
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','yingyangzhuanjia');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','yingyangzhuanjia');
			...
			async init(type=1){
				...
				let res = await this.$api.info('yingyangzhuanjia', this.id);
				if (res.data.touxiang) {
					this.fpic = res.data.touxiang.split(',')[0]
				} else if (res.data.headportrait) {
					this.fpic = res.data.headportrait.split(',')[0]
				}
				if(res.data.yingyangshizhanghao){
				...
				if(type==2){
					...
					await this.$api.update('yingyangzhuanjia',this.detail)
				}
			},
