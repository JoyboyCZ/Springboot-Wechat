	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyinshijianyi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyinshijianyi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyinshijianyi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyinshijianyi`, this.ruleForm);
						}
						uni.setStorageSync('discussyinshijianyiCleanType',true);
						if(this.cross){
