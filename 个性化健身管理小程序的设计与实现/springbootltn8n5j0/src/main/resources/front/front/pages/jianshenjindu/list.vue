			...
			<view :style='{"minHeight":"100%","width":"100%","padding":"0 20rpx","position":"relative","background":"#fff","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"padding":"20rpx 0 0 0","margin":"0","background":"none","display":"flex","width":"100%","position":"relative","height":"auto"}'>
					<view :style='{"border":"none","margin":"0","borderRadius":"20rpx","flex":"1","background":"none","lineHeight":"80rpx","position":"relative","height":"80rpx"}' class="search-form round">
						...
						<input placeholder-style="color: #fff" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#fff","borderRadius":"40rpx","background":"rgba(139, 125, 165, 0.5)","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.jihuabiaoti" type="text" placeholder="计划标题" ></input>
					</view>
				...
				<view :style='{"width":"100%","background":"none","flexDirection":"column","display":"flex","height":"auto"}'>
					...
					<view class="uni-product-list" :style='{"padding":"0","margin":"40rpx 0 0","flexWrap":"wrap","flex":"auto","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #ddd","boxShadow":"none","padding":"30rpx","margin":"0 0 20rpx","backgroundColor":"transparent","overflow":"hidden","borderRadius":"180rpx 180rpx 10rpx 10rpx","flexWrap":"wrap","background":"#f6f6f6","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
							<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","overflow":"hidden","color":"#333","maxHeight":"96rpx","width":"100%","lineHeight":"48rpx","fontSize":"32rpx","order":"2"}'>{{product.jihuabiaoti}}</view>
							<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","overflow":"hidden","color":"#333","maxHeight":"96rpx","width":"100%","lineHeight":"48rpx","fontSize":"32rpx","order":"2"}'>{{product.yundongleixing}}</view>
							...
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjindu','修改')) || (!userid && isAuthFront('jianshenjindu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjindu','删除')) || (!userid && isAuthFront('jianshenjindu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjindu','修改')) || (!userid && isAuthFront('jianshenjindu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjindu','删除')) || (!userid && isAuthFront('jianshenjindu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jianshenjindu`, params);
                } else {
                    res = await this.$api.list(`jianshenjindu`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jianshenjindu', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jianshenjindu`, searchForm);
                } else {
                    res = await this.$api.list(`jianshenjindu`, searchForm);
                }
				// 如果是第一页数据置空
