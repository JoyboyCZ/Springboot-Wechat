			...
			<view :style='{"minHeight":"100%","width":"100%","padding":"0 20rpx","position":"relative","background":"#fff","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"padding":"20rpx 0 0 0","margin":"0","background":"none","display":"flex","width":"100%","position":"relative","height":"auto"}'>
					<view :style='{"border":"none","margin":"0","borderRadius":"20rpx","flex":"1","background":"none","lineHeight":"80rpx","position":"relative","height":"80rpx"}' class="search-form round">
						...
						<input placeholder-style="color: #fff" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#fff","borderRadius":"40rpx","background":"rgba(139, 125, 165, 0.5)","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.biaoti" type="text" placeholder="标题" ></input>
					</view>
				...
				<view :style='{"width":"100%","background":"none","flexDirection":"column","display":"flex","height":"auto"}'>
					...
					<view class="uni-product-list" :style='{"padding":"0","margin":"40rpx 0 0","flexWrap":"wrap","flex":"auto","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #ddd","boxShadow":"none","padding":"30rpx","margin":"0 0 20rpx","backgroundColor":"transparent","overflow":"hidden","borderRadius":"180rpx 180rpx 10rpx 10rpx","flexWrap":"wrap","background":"#f6f6f6","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
							<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","overflow":"hidden","color":"#333","maxHeight":"96rpx","width":"100%","lineHeight":"48rpx","fontSize":"32rpx","order":"2"}'>{{product.biaoti}}</view>
							<image :style='{"border":"20rpx solid #4D357B","padding":"0","margin":"0","objectFit":"cover","borderRadius":"50%","display":"block","width":"100%","height":"280rpx","order":"1"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.fengmian)" :src="product.fengmian.split(',')[0]"></image>
							<image :style='{"border":"20rpx solid #4D357B","padding":"0","margin":"0","objectFit":"cover","borderRadius":"50%","display":"block","width":"100%","height":"280rpx","order":"1"}' mode="aspectFill" class="uni-product-image" v-else :src="product.fengmian?baseUrl+product.fengmian.split(',')[0]:''"></image>
							...
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijianyi','修改')) || (!userid && isAuthFront('yinshijianyi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijianyi','删除')) || (!userid && isAuthFront('yinshijianyi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijianyi','修改')) || (!userid && isAuthFront('yinshijianyi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijianyi','删除')) || (!userid && isAuthFront('yinshijianyi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>建议类型</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="jianyileixingChange(item)" v-for="(item,index) in jianyileixingOptions" :key="index" class="screenTab" :class="searchForm.jianyileixing==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"18%","padding":"0 0 0 20rpx","fontSize":"24rpx"}'>封面</view>
						<input :style='{"border":"2rpx solid rgb(162, 144, 104)","width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","background":"none","height":"60rpx"}' placeholder="请输入封面" v-model="searchForm.fengmian">
					</view>
	...
	export default {
		...
		methods: {
			jianyileixingChange(e){
				this.searchForm.jianyileixing = e
				this.$forceUpdate()
			},
			screenReset(){
				this.searchForm = {}
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.jianyileixing){
					params['jianyileixing'] = '%' + this.searchForm.jianyileixing + '%'
				}
				if(this.searchForm.fengmian){
					params['fengmian'] = '%' + this.searchForm.fengmian + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`yinshijianyi`, params);
                } else {
                    res = await this.$api.list(`yinshijianyi`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yinshijianyi', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.jianyileixing){
					searchForm['jianyileixing'] = '%' + this.searchForm.jianyileixing + '%'
				}
				if(this.searchForm.fengmian){
					searchForm['fengmian'] = '%' + this.searchForm.fengmian + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`yinshijianyi`, searchForm);
                } else {
                    res = await this.$api.list(`yinshijianyi`, searchForm);
                }
				// 如果是第一页数据置空
