					...
					<view class="uni-product-list" :style='{"padding":"0","margin":"40rpx 0 0","flexWrap":"wrap","flex":"auto","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #ddd","boxShadow":"none","padding":"30rpx","margin":"0 0 20rpx","backgroundColor":"transparent","overflow":"hidden","borderRadius":"180rpx 180rpx 10rpx 10rpx","flexWrap":"wrap","background":"#f6f6f6","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjilu','修改')) || (!userid && isAuthFront('jianshenjilu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjilu','删除')) || (!userid && isAuthFront('jianshenjilu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjilu','修改')) || (!userid && isAuthFront('jianshenjilu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('jianshenjilu','删除')) || (!userid && isAuthFront('jianshenjilu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jianshenjilu`, params);
                } else {
                    res = await this.$api.list(`jianshenjilu`, params);
                }
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jianshenjilu`, searchForm);
                } else {
                    res = await this.$api.list(`jianshenjilu`, searchForm);
                }
				// 如果是第一页数据置空
