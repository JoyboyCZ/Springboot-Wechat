			...
			<view :style='{"minHeight":"100%","width":"100%","padding":"0 20rpx","position":"relative","background":"#fff","height":"auto"}'>
				...
				<view :style='{"width":"100%","background":"none","flexDirection":"column","display":"flex","height":"auto"}'>
					...
					<view class="uni-product-list" :style='{"padding":"0","margin":"40rpx 0 0","flexWrap":"wrap","flex":"auto","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #ddd","boxShadow":"none","padding":"30rpx","margin":"0 0 20rpx","backgroundColor":"transparent","overflow":"hidden","borderRadius":"180rpx 180rpx 10rpx 10rpx","flexWrap":"wrap","background":"#f6f6f6","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","overflow":"hidden","color":"#333","maxHeight":"96rpx","width":"100%","lineHeight":"48rpx","fontSize":"32rpx","order":"2"}'>{{product.riqi}}</view>
							<image :style='{"border":"20rpx solid #4D357B","padding":"0","margin":"0","objectFit":"cover","borderRadius":"50%","display":"block","width":"100%","height":"280rpx","order":"1"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.tupian)" :src="product.tupian.split(',')[0]"></image>
							<image :style='{"border":"20rpx solid #4D357B","padding":"0","margin":"0","objectFit":"cover","borderRadius":"50%","display":"block","width":"100%","height":"280rpx","order":"1"}' mode="aspectFill" class="uni-product-image" v-else :src="product.tupian?baseUrl+product.tupian.split(',')[0]:''"></image>
							...
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijilu','修改')) || (!userid && isAuthFront('yinshijilu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijilu','删除')) || (!userid && isAuthFront('yinshijilu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"padding":"8rpx 0","flexDirection":"column","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"8"}'>
								<view :style='{"width":"100%","padding":"8rpx 12rpx","background":"rgba(46, 34, 69, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijilu','修改')) || (!userid && isAuthFront('yinshijilu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"8rpx 12rpx","background":"rgba(34, 31, 40, 0.3)","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('yinshijilu','删除')) || (!userid && isAuthFront('yinshijilu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>早餐摄入区间</view>
						<view :style='{"width":"100%","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
							<input :style='{"border":"2rpx solid rgb(162, 144, 104)","borderRadius":"40rpx","textAlign":"center","background":"none","width":"40%","lineHeight":"60rpx","height":"60rpx"}' v-model="searchForm.zaocansherustart" placeholder="请输入开始早餐摄入">
							<view :style='{"padding":"0 10rpx","fontSize":"20rpx"}'>-</view>
							<input :style='{"border":"2rpx solid rgb(162, 144, 104)","borderRadius":"40rpx","textAlign":"center","background":"none","width":"40%","lineHeight":"60rpx","height":"60rpx"}' v-model="searchForm.zaocansheruend" placeholder="请输入结束早餐摄入">
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zaocansheru){
					params['zaocansheru'] = '%' + this.searchForm.zaocansheru + '%'
				}
			...
			async search(){
				...
				if(this.searchForm.zaocansherustart){
					searchForm['zaocansherustart'] = this.searchForm.zaocansherustart
				}
				if(this.searchForm.zaocansheruend){
					searchForm['zaocansheruend'] = this.searchForm.zaocansheruend
				}
                let res = {};
                if(this.userid) {
